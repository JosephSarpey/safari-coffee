...
import { Calendar, MessageCircle, User, Search, Tag, ArrowRight } from "lucide-react";
      ...
      <section className="section-padding container">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
            ...
            <div className="lg:col-span-2 space-y-8">
                ...
                {/* Comments UI (Static) */}
                <div className="pt-16">
                    <h3 className="text-2xl font-bold uppercase tracking-widest text-white mb-8">{post.comments} Comments</h3>
                    <ul className="space-y-8">
                         {[...Array(3)].map((_, i) => (
                             <li key={i} className="flex gap-4">
                                 <div className="h-12 w-12 bg-white/10 rounded-full flex-shrink-0" />
                                 <div className="space-y-2">
                                     <div className="flex items-center justify-between">
                                         <h4 className="text-white font-bold uppercase tracking-widest text-sm">John Doe</h4>
                                         <span className="text-xs text-gray-500 uppercase">Oct 0{i+1}, 2024</span>
                                     </div>
                                     ...
                                     <button className="text-xs uppercase font-bold tracking-widest text-gray-500 hover:text-primary">Reply</button>
                                 </div>
                             </li>
                         ))}
                    </ul>
                ...
                {/* Comment Form */}
                <div className="pt-16">
                    <h3 className="text-2xl font-bold uppercase tracking-widest text-white mb-8">Leave a Comment</h3>
                    <form className="space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                             <div className="space-y-1">
                                <label className="text-xs font-bold uppercase tracking-widest text-white/60">Name *</label>
                                <input type="text" className="w-full bg-transparent border border-white/10 p-4 text-white focus:border-primary outline-none transition-colors" />
                             </div>
                             <div className="space-y-1">
                                <label className="text-xs font-bold uppercase tracking-widest text-white/60">Email *</label>
                                <input type="email" className="w-full bg-transparent border border-white/10 p-4 text-white focus:border-primary outline-none transition-colors" />
                             </div>
                        </div>
                        <div className="space-y-1">
                            <label className="text-xs font-bold uppercase tracking-widest text-white/60">Website</label>
                            <input type="url" className="w-full bg-transparent border border-white/10 p-4 text-white focus:border-primary outline-none transition-colors" />
                        </div>
                        <div className="space-y-1">
                            <label className="text-xs font-bold uppercase tracking-widest text-white/60">Message</label>
                            <textarea rows={6} className="w-full bg-transparent border border-white/10 p-4 text-white focus:border-primary outline-none transition-colors" />
                        </div>
                        <button className="btn-primary px-8 py-4">Post Comment</button>
                    </form>
                </div>
                         ...
                         {recentPosts.map(post => (
                             <div key={post.id} className="flex gap-4 group cursor-pointer">
                                 <div className="relative h-20 w-20 flex-shrink-0 overflow-hidden">
                                     <Image src={post.image} alt={post.title} fill className="object-cover group-hover:scale-110 transition-transform duration-500" sizes="80px" />
                                 </div>
                                 <div className="space-y-1">
