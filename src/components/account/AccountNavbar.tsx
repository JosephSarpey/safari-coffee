...
export function AccountNavbar() {
  ...
  useEffect(() => {
    const loadProfile = async () => {
      try {
        // Try reading from localStorage first for immediate display to avoid layout shift, 
        // though strictly we should rely on API. 
    ...
    <header className="fixed top-0 left-0 right-0 z-50 h-16 bg-[#0a0a0a] border-b border-white/10 flex items-center justify-between px-4 md:px-6">
      ...
      <div className="flex items-center gap-4 md:gap-8">
        <div className="flex items-center gap-2 group">
         {/* Replace with logo */} 
         <Link href="/"> 
             <div className="w-8 h-8 rounded-full bg-[#c49b63] flex items-center justify-center text-black font-bold font-serif text-sm md:text-lg">
               S
             </div>
           </Link>
           <div className="flex flex-col">
             <Link href="/" className="font-serif font-bold text-white text-base md:text-lg tracking-wide group-hover:text-[#c49b63] transition-colors leading-tight">
               SAFARI <span className="text-[#c49b63]">ROAST</span>
             </Link>
             <Link href="/" className="text-[10px] text-stone-500 hover:text-[#c49b63] -mt-1 md:hidden font-medium">
               Visit Website
             </Link>
           </div>
        </div>
